...
public class ClienteService {
	...
	public Cliente findById(Integer id) {
		...
		return obj.orElseThrow(() -> new ObjectNotFoundException("Object não encontrado!: " + id));
	}
	...
	private void validaPorCpfEEmail(ClienteDTO objDto) {
		...
		obj = pessoaRepository.findByEmail(objDto.getEmaill());
		if (obj.isPresent() && obj.get().getId() != objDto.getId()) {
	...
	public void delete(Integer id) {
		...
		if (obj.getChamados().size() > 0) {
			throw new DataIntegrityViolationException("O técnico possui ordens de chamados e não pode ser excluido.");
		}

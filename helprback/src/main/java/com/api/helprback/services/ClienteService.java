...
public class ClienteService {
    ...
    private void validaPorCpfEEmail(ClienteDTO objDto) {
        ...
        if (obj.isPresent() && obj.get().getId() != objDto.getId()) {
            throw new DataIntegrityViolationException("CPF já cadastrado no sistma!");
        }
        obj = pessoaRepository.findByEmail(objDto.getEmail());
        if (obj.isPresent() && obj.get().getId() != objDto.getId()) {
            throw new DataIntegrityViolationException("E-mail já existe no sistema");
        }
    }
    ...
    public void delete(Integer id) {
        ...
        if (obj.getChamados().size() > 0) {
            throw new DataIntegrityViolationException(" O cliente possui ordens de chamados e não pode ser excluído.");
        }

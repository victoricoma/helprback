...
SELECT * FROM log_empregado WHERE NOT ISNULL(depto_antigo);
...
DELIMITER //
CREATE PROCEDURE sps_verifica_log_depto(
IN dt_busca_init DATETIME,
IN dt_busca_fim DATETIME
)
BEGIN
    ...
    FROM log_empregado WHERE dt_alteracao BETWEEN dt_busca_fim AND dt_busca_init
    AND NOT ISNULL(depto_antigo);
END //
DELIMITER ;

CALL sps_verifica_log_salario(NOW(), "2022-07-29");
CALL sps_verifica_log_depto(NOW(), "2022-07-29");
...
BEGIN
    ...
    AS
    SELECT inss.cod_empregado, inss.empregado, inss.renda_bruta, inss.desconto, COUNT(dep.cod_dep)
